...
module BannersHelper
  ...
  def banner_for(category_name, options={})
    ...
    if options[:position] == :sidetop
      ...
      link_to(image_tag('catalog/banner_quiz_catalog.jpg'), new_survey_path , onclick: track_event('Catalog', "BottomBannerFrom#{Category.key_for(category_id).to_s.camelize}", wysquiz_path))
    else
      ""
    end
  end

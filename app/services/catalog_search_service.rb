class CatalogSearchService
  ...
  def search_products
    ...
    query_shoe_sizes = (query_subcategories || query_heels || query_colors) && params[:shoe_sizes] ? build_sub_query((query_colors || query_heels || query_subcategories || query_base), l_products[:shoe_size].in(params[:shoe_sizes])) : nil
    ...
    query_bags_acessories = build_sub_query(query_bags_acessories || query_base, Product.arel_table[:color_name].in(params[:bag_colors])) if params[:bag_colors]

    if query_bags_acessories
      query_bags_acessories = build_sub_query(query_bags_acessories, l_products[:category_id].in(Category::BAG))

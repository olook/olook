class SearchUrlBuilder
  ...
  def build_url_for(options={})
    ...
    q = @query ? "?#{@query}&" : "?"
    URI.parse("http://#{@base_url}#{q}#{bq}return-fields=subcategory,name,brand,image,retail_price,price,backside_image,category,text_relevance&size=99&start=#{ options[:start] }&rank=#{ @sort_price }-cor_e_marca&size=#{ options[:limit] }")
  end

...
class PaymentBuilder
  ...
  def process!
    ...
    ActiveRecord::Base.transaction do
      ...
      if @gateway_strategy.payment_successful?
        ...
        @payment.create_preferences(@cart.address, @cart_service.total_discount) if @payment.is_a? MercadoPagoPayment
  ...
  private
    def create_discount_payments
      total_promotion = @cart.total_promotion_discount
      ...
      total_coupon = @cart.total_coupon_discount
      coupon_opts = {:coupon_id => @cart.coupon_id}
      total_liquidation = @cart.total_liquidation_discount
      ...
      @cart.items.each do |item|
        if /Promotion:/ =~ item.cart_item_adjustment.source
          ...
          create_payment_for(item.adjustment_value, PromotionPayment, {line_item_id: line_item.try(:id), promotion: @cart.items.first.cart_item_adjustment.source})
        end
      end
      create_payment_for(total_promotion, PromotionPayment, {promotion: @cart.items.first.cart_item_adjustment.source})
    ...
    def notify_big_billet_sail _payment
      ...
      }) if is_a_big_billet_sail?(_payment)
    end

    def is_a_big_billet_sail?(_payment)
      _payment.class == Billet && _payment.total_paid > 1000
    end

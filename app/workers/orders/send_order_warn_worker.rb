...
class SendOrderWarnWorker
  ...
  def self.perform
    ...
    warn_orders = Order.select("number, created_at").where(state: "Authorized").where(erp_payment_at: nil).order("created_at DESC")
    DevAlertMailer.braspag_capture_warn(warn_payments, warn_orders).delivery unless @warn_payments.empty? && @warn_orders.empty?
  end
end

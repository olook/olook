...
describe BilletMailer do
  ...
  subject { BilletMailer.send_reminder_mail(order.payment) }
  ...
  describe '#send_reminder_mail' do
    ...
    it "should contain users name and billet expiration date in the body" do
      ...
      subject.body.should include(order.payment.payment_expiration_date.strftime('%d/%m/%Y'))
    end

    context "when not expiring on monday" do
      before do
        order.payment.payment_expiration_date = Time.new(2012,7,3)
      end
    ...
    context "when expiring on monday" do
      before do
        order.payment.payment_expiration_date = Time.new(2012,7,2)
      end

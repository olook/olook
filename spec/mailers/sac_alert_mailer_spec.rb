...
describe SACAlertMailer do
  ...
  describe "#fraud_analysis_notification" do
    subject { SACAlertMailer.fraud_analysis_notification(order, to) }
    ...
    it "should assign correct subject" do
      subject.subject.should eq("Pedido: #{order.number} | Análise de Fraude")
    end
    ...
    it "should containt payments details" do
      order.payments.each do |payment|
        subject.body.should include(payment.type)
        subject.body.should include(number_to_currency(payment.total_paid))
        subject.body.should include(payment.url)
      end
    end
  end
end

...
describe CouponManager do
  ...
  context "removing a coupon" do
    context "when a order has a used coupon" do
      it "should remove it" do
        subject.apply_coupon
        expect {
          subject.remove_coupon
        }.to change(UsedCoupon, :count).by(-1)
      end
    end

    context "when a order dont have a used coupon" do
      it "should not remove it" do
        expect {
          subject.remove_coupon

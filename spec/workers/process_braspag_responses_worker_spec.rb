...
describe ProcessBraspagResponsesWorker do
  ...
  context "when payment exists" do
    ...
    let!(:authorize_response) { FactoryGirl.create(:braspag_authorize_response, :order_id => payment.identification_code) }
    let!(:capture_response) { FactoryGirl.create(:braspag_capture_response, :order_id => payment.identification_code ) }

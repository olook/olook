...
describe Promotions::PurchasesAmountStrategy do
  ...
  it "should be applied if the user matches the requirement" do
    Promotions::PurchasesAmountStrategy.new(promo, FactoryGirl.create(:user)).matches?.should be_true
  end

  it "should not be applied if the user matches the requirement" do
    Promotions::PurchasesAmountStrategy.new(promo, user_with_order_delivered ).matches?.should be_false
  end

...
namespace :coupons do
  ...
  task :create_batch, [:filename, :col_sep] => :environment do |t, args|
    ...
    begin
      ...
      csv.each do |row|
        Coupon.create(:code => row[:code], :is_percentage => row[:is_percentage], :value => row[:value], :start_date => row[:start_date], :end_date => row[:end_date].to_s, :remaining_amount => row[:remaining_amount], :unlimited => row[:unlimited], :active => row[:active], :campaign => row[:campaign], :campaign_description => row[:campaign_description], :created_by => row[:created_by] )
      end
    rescue Exception => e
  ...
  def check_csv_syntax(header)
    excluded_fields = [:created_at, :updated_at, :id, :used_amount, :updated_by]

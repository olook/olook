namespace :marketing_uploader do
  ...
  task :copy_userbase_to_ftp => :environment do
    EmailMarketing::CsvUploader.new(:userbase).copy_to_ftp("base_atualizada_purchases.csv")
  end
  ...
  task :copy_userbase_revenue_to_ftp => :environment do
    EmailMarketing::CsvUploader.new(:userbase_revenue).copy_to_ftp("base_receita_atualizada.csv")
  end

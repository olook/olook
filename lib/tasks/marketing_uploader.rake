namespace :marketing_uploader do
  ...
  task :copy_email_to_ftp => :environment do
    MarketingReports::CsvUploader.new(:invalid).copy_to_ftp("invalid_email.csv")
    MarketingReports::CsvUploader.new(:optout).copy_to_ftp("optout_email.csv")
  end

  desc "Uploads a CSV file with all user data (user name, email, etc)"
  task :copy_userbase_to_ftp => :environment do
    MarketingReports::CsvUploader.new(:userbase).copy_to_ftp("base_atualizada_purchases.csv")
  end

  desc "Uploads a CSV with userbase orders"
  task :copy_userbase_orders_to_ftp => :environment do
    MarketingReports::CsvUploader.new(:userbase_orders).copy_to_ftp("base_pedidos_atualizada.csv")
  end

  desc "Uploads a CSV with userbase revenue"
  task :copy_userbase_revenue_to_ftp => :environment do
    MarketingReports::CsvUploader.new(:userbase_revenue).copy_to_ftp("base_receita_atualizada.csv")
  end

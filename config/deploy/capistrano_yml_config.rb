...
Capistrano::Configuration.instance.load do
  ...
  namespace :deploy do
    ...
    namespace :yml do
      ...
      desc <<-DESC
        Creates the configurations in yaml files such as
        database.yml configuration file in shared path.

        You need to create a database.yml.erb (and other) either in :template_dir \
        or /config/deploy folders.

        When this recipe is loaded, yml:setup is automatically configured \
        to be invoked after deploy:setup. You can skip this task setting \
        the variable :skip_yml_setup to true. This is especially useful \
    ...
    after "deploy:setup",           "deploy:yml:setup"   unless fetch(:skip_db_setup, false)
    after "deploy:finalize_update", "deploy:yml:symlink"
